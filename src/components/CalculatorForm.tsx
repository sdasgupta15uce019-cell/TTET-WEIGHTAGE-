            ...
            {isConnecting ? (
              <div className="flex flex-col items-center justify-center py-12 space-y-8">
                ...
                <div className="space-y-3 text-center">
                  <p className="text-zinc-900 font-black text-2xl animate-pulse">Connecting to TRBT server</p>
                  <p className="text-zinc-500 font-bold text-lg">Fetching and verifying data...</p>
                </div>
              </div>
            ) : (
              <div className="w-full space-y-8 py-12">
                <div className="text-center space-y-4">
                  <div className="w-20 h-20 bg-emerald-100 text-emerald-600 rounded-3xl flex items-center justify-center mx-auto border border-emerald-200 shadow-sm">
                    <CheckCircle className="w-10 h-10" />
                  </div>
                  <h4 className="text-3xl text-zinc-900 font-black tracking-tight">Calculation Successful!</h4>
                  <p className="text-xl text-emerald-700 font-bold">Name: {submittedResult.name}</p>
                </div>
                ...
                <div className="flex flex-wrap justify-center gap-3">
                  ...
                  {submittedResult.scoreTET2 >= 90 && (submittedResult.category === 'SC' || submittedResult.category === 'ST' || submittedResult.category === 'PH') && (
                    <div className="px-4 py-2 bg-emerald-50 border border-emerald-100 text-emerald-700 text-xs font-black rounded-full uppercase tracking-widest shadow-sm">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {submittedResult.allRank !== null && (
                    <div className="bg-zinc-50 border border-zinc-100 p-6 rounded-3xl shadow-sm flex flex-col items-center justify-center text-center">
                      <p className="text-xs text-zinc-500 font-bold uppercase tracking-widest mb-2">All Rank</p>
                      <p className="text-4xl font-black text-zinc-900">#{submittedResult.allRank}</p>
                    </div>
                  )}
                  <div className="bg-zinc-50 border border-zinc-100 p-6 rounded-3xl shadow-sm flex flex-col items-center justify-center text-center">
                    <p className="text-xs text-zinc-500 font-bold uppercase tracking-widest mb-2">Category Rank</p>
                    <p className="text-4xl font-black text-zinc-900">#{submittedResult.categoryRank}</p>
                  </div>
                  <div className="bg-emerald-50 border border-emerald-100 p-6 rounded-3xl shadow-sm flex flex-col items-center justify-center text-center">
                    <p className="text-xs text-emerald-700 font-bold uppercase tracking-wider mb-2">Weightage</p>
                    <p className="text-4xl font-black text-emerald-900">{submittedResult.score.toFixed(2)}</p>
                  </div>
                  <div className="bg-zinc-50 border border-zinc-100 p-6 rounded-3xl shadow-sm flex flex-col items-center justify-center text-center">
                    <p className="text-xs text-zinc-500 font-bold uppercase tracking-widest mb-2">TET Score</p>
                    <p className="text-4xl font-black text-zinc-900">{submittedResult.scoreTET2}</p>
                  </div>
                  <div className="bg-zinc-50 border border-zinc-100 p-6 rounded-3xl shadow-sm flex flex-col items-center justify-center text-center sm:col-span-2">
                    <p className="text-xs text-zinc-500 font-bold uppercase tracking-widest mb-2">Category</p>
                    <p className="text-3xl font-black text-zinc-900">{submittedResult.category}</p>
                  </div>
                </div>
              </div>
            )}
        ...
        <div className="fixed inset-0 z-[100] bg-white flex flex-col animate-in fade-in slide-in-from-bottom duration-500">
          ...
          >
            <div className="w-full space-y-8 py-12">
              ...
              <div className="bg-zinc-50 border border-zinc-100 rounded-3xl p-8 space-y-6 shadow-sm">
                <div className="space-y-4">
                  <div className="flex flex-col items-center border-b border-zinc-200 pb-4 text-center">
                    <span className="text-xs font-bold text-zinc-500 uppercase tracking-widest mb-1">Name</span>
                    <span className="text-2xl font-black text-zinc-900 flex items-center gap-2">
                      {duplicateRecord.name}
                      {duplicateRecord.isVerified === true && <CheckCircle className="w-6 h-6 text-emerald-500" />}
                      {duplicateRecord.isVerified === false && <XCircle className="w-6 h-6 text-red-500" />}
                    </span>
                  </div>
                  <div className="flex justify-between items-center border-b border-zinc-200 pb-4">
                    <span className="text-xs font-bold text-zinc-500 uppercase tracking-widest">Category</span>
                    <span className="text-xl font-black text-zinc-900">{duplicateRecord.category}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-xs font-bold text-zinc-500 uppercase tracking-widest">Weightage</span>
                    <span className="text-2xl font-black text-emerald-600">{duplicateRecord.finalScore.toFixed(2)}</span>
                  </div>
                </div>
                
                <div className="grid grid-cols-2 gap-4 pt-4">
                  {getDuplicateRanks(duplicateRecord).allRank !== null && (
                    <div className="bg-white p-4 rounded-2xl text-center border border-zinc-200 shadow-sm">
                      <span className="block text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-1">All Rank</span>
                      <span className="text-2xl font-black text-zinc-900">#{getDuplicateRanks(duplicateRecord).allRank}</span>
                    </div>
                  )}
                  <div className="bg-white p-4 rounded-2xl text-center border border-zinc-200 shadow-sm">
                    <span className="block text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-1">Category Rank</span>
                    <span className="text-2xl font-black text-zinc-900">#{getDuplicateRanks(duplicateRecord).categoryRank}</span>
                  </div>
                </div>
              </div>
              ...
              {duplicateRecord.isVerified !== true && (onVerify || onVerifyBySlNo) && (
                <div className="space-y-4">
                  <h4 className="text-sm font-black text-zinc-900 uppercase tracking-widest text-center">Verify Your Entry</h4>
                  <div className="flex flex-col gap-3">
                    <input
                      ...
                      className="w-full px-6 py-4 rounded-2xl border-2 border-zinc-100 focus:border-emerald-500 focus:outline-none text-lg font-bold transition-all"
                    />
                    <button
                      type="button"
                      onClick={() => {
